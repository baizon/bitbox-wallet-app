...
func (s *addressTestSuite) TestNewAddress() {
	s.Require().Equal(s.T(), absoluteKeypath, s.address.Configuration.AbsoluteKeypath())
	s.Require().Equal(s.T(), "n2gAErwJCuPmnQuhzPkkWi2haGz9oQxjnX", s.address.EncodeAddress())
...
func (s *addressTestSuite) TestPubkeyScript() {
	...
	s.Require().Equal(s.T(), payToAddrScript, s.address.PubkeyScript())
}
